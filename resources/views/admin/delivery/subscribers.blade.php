                            ...
                            <table id="example1" class="table table-bordered table-striped">
                                ...
                                @foreach ($subscribers as $subscriber)
                                    <tr>
                                        ...
                                        <td>{{$subscriber['ip_opt']}}</td>
                                        <td>
                                            <center>
                                                <div class="btn-group text-center">
                                                    <a href='{{URL::to('/delivery/subscribers/edit/'.$subscriber['id'])}}'
                                                       class="btn btn-warning btn-xs">редактировать</a>
                                                    <button type="button" data-id="{{$subscriber['email_address']}}"
            ...
            $('body').on('click', '.remove', function (event) {
                ...
                bootbox.confirm("Действительно хотите удалить клиента и его заказы?", function (result) {
                    if (result == true) {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: SYS_URL + '/delivery/subscribers/unsubscribe/' + id,
                            data: data,
                            //dataType: 'html',
                            success: function (html) {
                                window.location = SYS_URL + '/delivery/subscribers'
                            }
                        });
                    }
                    else {
